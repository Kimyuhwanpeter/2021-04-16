...
lr_scheduler = LinearDecay_(FLAGS.lr, FLAGS.epochs * len_dataset, 15 * len_dataset)
...
def cal_mae(model, images, labels):
    ...
    max_feature = tf.nn.softmax(logits, 1)    # [None, 9]
...
def main():
    model = modified_GEI_model(input_shape=(FLAGS.img_height, FLAGS.img_width, 1), weight_decay=0.00001)
    model.summary()
